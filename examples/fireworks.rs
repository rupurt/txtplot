...
/// Demo: Fireworks / particle system (PRO autoadjust)
///
/// Controls:
///   - q / Esc : quit
///
...
///   - This example stresses: per-pixel drawing, color buffering, animation.
...
/// PRO: generates a rocket whose apex lands inside a visible terminal band.
/// Keeps the demo model intact: velocity is roughly px/frame, position integrates with *dt*60.
fn spawn_rocket_pro(rng: &mut ThreadRng, w_px: usize, h_px: usize) -> Rocket {
    ...
    // We want the apex to land in a visible, aesthetically pleasing band.
    // (0,0) is top-left, so a smaller target_apex_y means a higher burst.
    let band_top = (h_px as f64) * 0.18; // 18%
    let band_bottom = (h_px as f64) * 0.38; // 38%
    let target_apex_y = rng.gen_range(band_top..band_bottom);

    // Per-frame physics (assuming ~60 FPS as the reference for vy/fuse):
    let dt_assumed = 1.0 / 60.0;
    let gravity = 18.0; // como en el loop principal
    let g_frame = gravity * dt_assumed * 0.55; // lo mismo que r.vy += gravity*dt*0.55
...
fn main() -> io::Result<()> {
    ...
    let mut rockets: Vec<Rocket> = Vec::with_capacity(32);
    let mut particles: Vec<Particle> = Vec::with_capacity(12_000);
    ...
    while running {
        ...
        // Explode rockets (PRO: failsafe ceiling)
        let mut i = 0;
        ...
        let hud = format!(
            "FIREWORKS (PRO) | rockets={} particles={} | {:02}:{:02} | q/Esc quit",
            rockets.len(),
            particles.len(),
            uptime / 60,
            uptime % 60
        ...
        chart.text(
            "Auto-apex band: 18%..38% height | Failsafe ceiling explode",
            0.02,
            0.12,
        ...
        let output = chart
            .canvas
            .render_with_options(true, Some("PARTICLE SYSTEM — FIREWORKS (PRO)"));
